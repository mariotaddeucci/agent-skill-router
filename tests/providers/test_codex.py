"""Tests for the Codex provider scope flags."""
...
async def test_codex_provider_workspace_root_used(all_disabled_settings, tmp_path):
    ws = tmp_path / ".codex" / "skills"
    skill = ws / "ws-skill"
    ...
    patched = [
        (attr, cls, roots)
        if attr != "enable_codex"
        else (
            "enable_codex",
            SkillsDirectoryProvider,
            {
                "workspace": [ws],
                "user": [tmp_path / ".codex" / "user-skills"],
            },
        )
...
async def test_codex_provider_user_root_used(all_disabled_settings, tmp_path):
    user = tmp_path / ".codex" / "user-skills"
    ...
    patched = [
        ...
        if attr != "enable_codex"
            ...
            SkillsDirectoryProvider,
            {
                "workspace": [tmp_path / ".codex" / "ws-skills"],
                "user": [user],
            },
        )
        for attr, cls, roots in original
...
async def test_codex_provider_disabled_exposes_nothing(all_disabled_settings):
...
def test_codex_provider_class_registered():
    """Ensure the Codex entry in _PROVIDER_ROOTS uses CodexSkillsProvider."""
    entry = next(e for e in srv._PROVIDER_ROOTS if e[0] == "enable_codex")
    assert entry[1] is CodexSkillsProvider
