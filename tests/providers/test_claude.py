"""Tests for the Claude provider scope flags."""
...
async def test_claude_provider_workspace_root_used(all_disabled_settings, tmp_path):
    ws = tmp_path / ".claude" / "skills"
    skill = ws / "ws-skill"
    ...
    patched = [
        (attr, cls, roots)
        if attr != "enable_claude"
        else (
            "enable_claude",
            SkillsDirectoryProvider,
            {
                "workspace": [ws],
                "user": [tmp_path / ".claude" / "user-skills"],
            },
        )
...
async def test_claude_provider_user_root_used(all_disabled_settings, tmp_path):
    user = tmp_path / ".claude" / "user-skills"
    ...
    patched = [
        ...
        if attr != "enable_claude"
            ...
            SkillsDirectoryProvider,
            {
                "workspace": [tmp_path / ".claude" / "ws-skills"],
                "user": [user],
            },
        )
        for attr, cls, roots in original
...
async def test_claude_provider_disabled_exposes_nothing(all_disabled_settings):
...
def test_claude_provider_class_registered():
    """Ensure the Claude entry in _PROVIDER_ROOTS uses ClaudeSkillsProvider."""
    entry = next(e for e in srv._PROVIDER_ROOTS if e[0] == "enable_claude")
    assert entry[1] is ClaudeSkillsProvider
